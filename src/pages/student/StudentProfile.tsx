...
import { UserCircle, BookOpen, MapPin, Phone, Mail, Save, Download, Loader2 } from 'lucide-react';
        ...
        <div className="px-8 pb-8">
          <div className="relative flex justify-between items-end -mt-16 mb-6">
            <div className="flex items-end gap-6">
              <div className="w-32 h-32 rounded-full border-4 border-white dark:border-slate-800 bg-white shadow-md overflow-hidden flex items-center justify-center text-slate-300">
                {avatarUrl ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8">
            <div className="space-y-6">
              <div>
                <h3 className="text-lg font-semibold text-slate-800 dark:text-white flex items-center gap-2 mb-4 pb-2 border-b border-slate-100 dark:border-slate-700">
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-3 text-sm">
                    ...
                    <span className="col-span-2 font-medium text-slate-900 dark:text-white">{student?.firstName || '—'}</span>
                  </div>
                  <div className="grid grid-cols-3 text-sm">
                    <span className="text-slate-500">Last Name</span>
                    <span className="col-span-2 font-medium text-slate-900 dark:text-white">{student?.lastName || '—'}</span>
                  </div>
                  <div className="grid grid-cols-3 text-sm">
                    <span className="text-slate-500">Date of Birth</span>
                    <span className="col-span-2 font-medium text-slate-900 dark:text-white">{student?.dateOfBirth || '—'}</span>
                  </div>
                  <div className="grid grid-cols-3 text-sm">
                    <span className="text-slate-500">Gender</span>
                    <span className="col-span-2 font-medium text-slate-900 dark:text-white">{student?.gender || '—'}</span>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-3 text-sm">
                    ...
                    <span className="col-span-2 font-medium text-slate-900 dark:text-white">{student?.enrollmentNo || '—'}</span>
                  </div>
                  <div className="grid grid-cols-3 text-sm">
                    <span className="text-slate-500">Department</span>
                    <span className="col-span-2 font-medium text-slate-900 dark:text-white">{student?.department || '—'}</span>
                  </div>
                  <div className="grid grid-cols-3 text-sm">
                    <span className="text-slate-500">Current Year</span>
                    <span className="col-span-2 font-medium text-slate-900 dark:text-white">{student ? `Year ${student.year}` : '—'}</span>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input type="email" disabled value={user?.email || ''}
                    className="w-full p-2.5 border border-slate-200 dark:border-slate-600 dark:bg-slate-900 rounded-lg text-slate-500 text-sm cursor-not-allowed" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1 flex items-center gap-2">
                  ...
                  <input type="tel"
                    ...
                    className="w-full p-2.5 border border-slate-200 dark:border-slate-600 dark:bg-slate-900 dark:text-white rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-sm" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1 flex items-center gap-2">
